<?php
	...
	$periodOfBalanceDateFrom = date_format($firstDayOfThisMonth, 'Y-m-d');
	$periodOfBalanceDateTo = date_format($lastDayOfThisMonth, 'Y-m-d');
	...
	try
	{
		...
		{
			if(isset($_POST['periodOfBalance']))
			{
				$selectedPeriod = $_POST['periodOfBalance'];
				...
				{
					$periodOfBalanceDateFrom =  date_format($firstDayOfLastMonth, 'Y-m-d');
					$periodOfBalanceDateTo =  date_format($lastDayOfLastMonth, 'Y-m-d');
				}
				else if($selectedPeriod == 'bieżący rok')
				{
					$periodOfBalanceDateFrom =  date_format($firstDayOfThisYear, 'Y-m-d');
					$periodOfBalanceDateTo =  date_format($lastDayOfThisMonth, 'Y-m-d');
				}
				else
				{
					if(isset($_POST['inputDateFrom']))
					{
						...
						{
							$periodOfBalanceDateFrom = $firstEnteredDate;
							$periodOfBalanceDateTo = $secondEnteredDate;
						}
						else
						{
							$periodOfBalanceDateFrom = $secondEnteredDate;
							$periodOfBalanceDateTo = $firstEnteredDate;
						}	
					}
				}
			}
			...
			$sqlFirst = "SELECT icatu.name, SUM(incomes.amount) AS suma_przychodow FROM incomes INNER JOIN incomes_category_assigned_to_users icatu ON icatu.id=incomes.income_category_assigned_to_user_id WHERE incomes.user_id='$userId' AND incomes.date_of_income BETWEEN '$periodOfBalanceDateFrom' AND '$periodOfBalanceDateTo'GROUP BY icatu.name ORDER BY suma_przychodow DESC ";
			$resultOfFirstSelect = mysqli_query($connection, $sqlFirst);
			
			$sqlSecond = "SELECT ecatu.name, SUM(expenses.amount) AS suma_wydatkow FROM expenses INNER JOIN expenses_category_assigned_to_users ecatu ON ecatu.id=expenses.expense_category_assigned_to_user_id WHERE expenses.user_id='$userId' AND expenses.date_of_expense BETWEEN '$periodOfBalanceDateFrom' AND '$periodOfBalanceDateTo' GROUP BY ecatu.name ORDER BY suma_wydatkow DESC";
			$resultOfSecondSelect = mysqli_query($connection, $sqlSecond);
			
			$sqlThird = "SELECT ecatu.name, SUM(expenses.amount) AS suma_wydatkow FROM expenses INNER JOIN expenses_category_assigned_to_users ecatu ON ecatu.id=expenses.expense_category_assigned_to_user_id WHERE expenses.user_id='$userId' AND expenses.date_of_expense BETWEEN '$periodOfBalanceDateFrom' AND '$periodOfBalanceDateTo' GROUP BY ecatu.name ORDER BY suma_wydatkow DESC";
			$resultOfThirdSelect = mysqli_query($connection, $sqlThird);
...
<head runat="server">
	...
	<script>
		...
		function drawChart() {
			var data = google.visualization.arrayToDataTable([
				...
				<?php
					while($row = $resultOfThirdSelect->fetch_assoc())
					{
			...
			var options = {
				...
				legend:{position:'top',alignment:'center',maxLines:10,textStyle:{fontSize:sizeOfFont}}
			};
    ...
    <script>
		function change(){
			localStorage.setItem('selectedPeriod', document.getElementById("periodForm").value);
			var type = $('#periodForm').find("option:selected").val();
            if (type.toUpperCase() == 'NIESTANDARDOWY') {
                $("#customPeriodWindow").modal("show");
            }
			else
			{
...
<body class="text-center d-flex flex-column min-vh-100">
	...
	<main class="container-fluid">
		<div class="container-of-balance">
				...
				<div class="col-sm-2">
					<form id="periodForm" method="post">
						<label class="select" for="type">
							<select id="periodOfBalance" name="periodOfBalance" class="select" required="required" onchange="change()">
				
								<option value="" disabled="disabled" selected="selected">
									<?php
										if(isset($_POST['periodOfBalance']))
										{
											$periodOfBalance = $_POST['periodOfBalance'];
											unset($_POST['periodOfBalance']);
											echo $periodOfBalance;
										}
										else 
										{
											echo "Wybierz okres";
						...
						<div id="customPeriodWindow" class="modal fade" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
							<div class="modal-dialog" role="document">
								<div class="modal-content">
									...
									<div class="modal-footer justify-content-center">
										<button type="button" class="likeButton btn-md me-sm-3" data-bs-dismiss="modal">Zamknij</button>
			...
			<?php
				if ($resultOfFirstSelect->num_rows > 0) 
				{
					$totalSumOfIncomes = 0;
					$sumOfIncomes = 0;
					echo '<table class="table incomes"><tr><th>Kategoria</th><th>Przychód [zł]</th></tr>';
					
				    while($row = $resultOfFirstSelect->fetch_assoc()) 
					{
			...
			<table class="table table-dark">
				<thead>
					<tr>
						<th class="table-dark">Bilans [zł]</th> 
						<td class="table-dark">
							<?php
								$balance = 0;
			...
			<div class="row">
				<?php
					...
					{
						echo '<label class="pb-3">Twoja sytuacja finansowa w wybranym okresie nie jest dobra!</label>';
					}
				?>
			</div>
			...
			<div class="row">
				<div class="col">
					<?php
						if ($resultOfThirdSelect->num_rows > 0) 
						{
